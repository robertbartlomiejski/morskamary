...
## CI-Green Pre-Commit Sequence (Mandatory)

For every change that modifies scripts, domain models (including QMBD matrix/axis logic), or any substantive artifact, run this exact sequence before finalizing a commit:

1. `python run_full_analysis.py` to regenerate dependent outputs in `outputs/`.
2. `python scripts/generate_manifest.py` to refresh `MANIFEST_SOURCES.csv`.
3. Append a fully formatted entry in `CHANGELOG.txt` documenting what changed and why.
4. Stage all generated/reconciled files: `outputs/`, `MANIFEST_SOURCES.csv`, and `CHANGELOG.txt`.
5. Verify no generator drift remains in outputs: `git diff --exit-code outputs/`.
6. Run baseline validation: `python -m pytest -q`.

Do not push a commit until the sequence above has been executed and reviewed.
