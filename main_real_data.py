...
def main():
    ...
    marine_comps = [
        c.id for c in mapper.get_competences_by_axis(BlueDynamicsAxis.MARINE)[:2]
    ]
    maritime_comps = [
        c.id for c in mapper.get_competences_by_axis(BlueDynamicsAxis.MARITIME)[:2]
    ]
    oceanic_comps = [
        c.id for c in mapper.get_competences_by_axis(BlueDynamicsAxis.OCEANIC)[:2]
    ]
    ...
    print("    Scenario: Worker transitions from Fisheries to Renewable Energy")
    print("    Currently has: [Sustainable resource management, Blue systems thinking]")
    ...
    print(
        f"   • Blue Economy sectors covered: {len(set(c.sector for c in mapper.credentials.values()))}"
    )
    print(
        f"   • MARINE competences: {len(mapper.get_competences_by_axis(BlueDynamicsAxis.MARINE))}"
    )
    print(
        f"   • MARITIME competences: {len(mapper.get_competences_by_axis(BlueDynamicsAxis.MARITIME))}"
    )
    print(
        f"   • OCEANIC competences: {len(mapper.get_competences_by_axis(BlueDynamicsAxis.OCEANIC))}"
    )
    print()
    print("💡 Next Steps:")
