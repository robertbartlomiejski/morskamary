...
from src.core import BlueDynamicsAxis, Competence, CompetenceLevel, MicroCredential  # noqa: E402
from src import literature_extractor, gap_analyzer, credential_designer, report_generator  # noqa: E402
...
def load_baseline_competences() -> List[Competence]:
    ...
    for _, row in df.iterrows():
        ...
        description = str(row.get("Key Simplified Focus (Applied to all 12 Sectors)", "") or "").strip()
...
def main() -> None:
    ...
    log(f"Designed {len(all_credentials_flat)} credentials across {len(credentials_by_sector)} sectors")

    # Step 6: pathways
    log("Step 6: Calculating pathways …", "🗺️")
    pathways = _calculate_pathways(credentials_by_sector)
    log(f"Pathway graph: {len(pathways.get('nodes', []))} nodes, {len(pathways.get('edges', []))} edges")
