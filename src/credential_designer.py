...
def define_assessment(sector: str, axis_counts: Dict[str, int]) -> str:
    ...
    if dominant_pct < 0.5:
        return (
            "Portfolio of evidence + practical demonstration + reflective journal"
        )

    if dominant == "MARINE":
        return (
            "Fieldwork report + species identification portfolio + oral examination"
        )
    if dominant == "OCEANIC":
        return (
            "Policy brief + stakeholder engagement simulation + written examination"
        )
    # MARITIME
    return (
        "Technical project + digital skills assessment + industry case study"
    )
