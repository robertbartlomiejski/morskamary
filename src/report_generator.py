...
<style>
  ...
  .card { background: white; border: 1px solid #dee2e6; border-radius: 6px; padding: 16px; margin-bottom: 16px; box-shadow: 0 1px 4px rgba(0,0,0,.06); }
...
def generate_html_index(
    ...
    avg_gap = (
        sum(d.get("gap_pct", 0) for d in gap_results.values()) / max(1, len(gap_results))
    )
...
def generate_literature_html(
    ...
    _AXIS_LABEL = {"M": "Marine", "T": "Maritime", "O": "Oceanic"}
    _GITHUB_BASE = (
...
def generate_json_databases(
    ...
    comp_path.write_text(
        json.dumps([c.to_dict() for c in all_competences], ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    ...
    cred_path.write_text(
        json.dumps([c.to_dict() for c in all_credentials], ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
...
def generate_csv_exports(
    ...
    with path.open("w", newline="", encoding="utf-8") as fh:
        writer = csv.DictWriter(
            fh,
            fieldnames=["sector", "required", "available", "missing", "gap_pct", "dominant_axis"],
        )
        writer.writeheader()
        for sector, data in gap_results.items():
            axis_bk = data.get("axis_breakdown", {})
