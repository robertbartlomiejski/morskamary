...
class TestWorkflowStructure:
    ...
    def test_h2_map_step_declares_a_stable_step_id(self) -> None:
    ...
    def test_rebuild_layer5_step_passes_validated_supply_map_argument(self) -> None:
        steps = self._load_steps()
        rebuild_step = next(
            s for s in steps if s.get("name") == "Rebuild Layer 5 with validated supply map"
        )
        assert "--validated-supply-map outputs/h2_credential_supply_map.json" in rebuild_step["run"]
        assert "build_layer4_5_scientific_analysis.py" in rebuild_step["run"]
...
# ── 10. MANIFEST_SOURCES.csv tracks the new regression test file ──────────────


class TestManifestTracksNewTestFile:
    """Thread: 'Register new review-fix regression tests in the manifest'."""

    def test_manifest_lists_pr215_review_fixes_test(self) -> None:
        manifest_path = REPO_ROOT / "MANIFEST_SOURCES.csv"
        with manifest_path.open("r", encoding="utf-8", newline="") as handle:
            rows = list(csv.DictReader(handle))
        matches = [row for row in rows if row["file_name"] == "tests/test_pr215_review_fixes.py"]
        assert len(matches) == 1, "expected exactly one manifest row for this test file"
        row = matches[0]
        assert row["file_type"] == "script"
        assert row["text_available"] == "yes"


# ── 11. credential_supply_registry.csv schema gained validation_evidence_ids ──


class TestCredentialRegistryCsvSchema:
    """Thread: 'Add validation_evidence_ids column to the credential registry'."""

    def test_registry_csv_header_includes_validation_evidence_ids(self) -> None:
        registry_path = (
            REPO_ROOT / "data" / "validated" / "credential_supply_registry.csv"
        )
        ...
        assert "validation_evidence_ids" in fieldnames
        assert fieldnames.index("validation_evidence_ids") < fieldnames.index("notes")

    def test_registry_csv_rows_have_blank_validation_evidence_ids(self) -> None:
        registry_path = (
            REPO_ROOT / "data" / "validated" / "credential_supply_registry.csv"
        )
        with registry_path.open("r", encoding="utf-8", newline="") as handle:
            rows = list(csv.DictReader(handle))
        assert rows, "expected placeholder rows in the credential registry"
        for row in rows:
