...
class TestFingerprintQueryIdHash:
    ...
    def test_normalize_query_constraints_hash_is_order_independent(self) -> None:
        """Query ID hash must be stable regardless of declaration order."""
        mod = _load_script("build_run_stability_report")
        forward = mod._normalize_query_constraints(
            {"queries": [{"query_id": "Q001"}, {"query_id": "Q002"}]}
        )
        reverse = mod._normalize_query_constraints(
            {"queries": [{"query_id": "Q002"}, {"query_id": "Q001"}]}
        )
        assert forward["query_id_hash"] == reverse["query_id_hash"]
        assert forward["query_id_hash"] != ""

    def test_normalize_query_constraints_empty_queries_yields_empty_hash(self) -> None:
        """No queries (or no query_id entries) means the hash stays empty."""
        mod = _load_script("build_run_stability_report")
        assert mod._normalize_query_constraints({})["query_id_hash"] == ""
        assert mod._normalize_query_constraints({"queries": []})["query_id_hash"] == ""
        assert (
            mod._normalize_query_constraints({"queries": [{"time_window": {}}]})[
                "query_id_hash"
            ]
            == ""
        )

    def test_fingerprint_defaults_empty_query_id_hash_to_unknown(self) -> None:
        """build_comparability_fingerprint must not silently ignore a missing hash."""
        mod = _load_script("build_run_stability_report")
...
class TestValidationEvidenceRequired:
    ...
    def test_validated_with_evidence_is_counted(self) -> None:
        """Positive-path counterpart: evidence-backed validated entries count."""
        mod = _load_script("build_validated_credential_supply_map")
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            reg_path = root / "registry.csv"
            ...
            with reg_path.open("w", encoding="utf-8", newline="") as f:
                ...
                writer.writerow({
                    ...
                    "axis_coverage": "HYDRONIZATION",
                    "validation_status": "validated",
                    "source_url": "https://example.org/cred-1",
                    "validation_evidence_ids": "EVD-100|EVD-101",
                    "notes": "",
                })
            ...
            registry_entries = mod.load_registry(reg_path)
            assert registry_entries[0].validation_evidence_ids == ("EVD-100", "EVD-101")
    ...
    def test_real_registry_file_parses_with_evidence_column(self) -> None:
        """Regression guard: the shipped registry CSV must load under the new schema."""
        mod = _load_script("build_validated_credential_supply_map")
        registry_path = (
        ...
        assert entries, "expected at least one row in the shipped registry"
        for entry in entries:
            # All shipped candidate rows are still pending human validation, so
            # none should have evidence ids yet, and validation_evidence_ids
            # must always resolve to a tuple (never raise on the new column).
...
class TestProtocolProjectionValidation:
    ...
    def test_blank_protocol_version_does_not_fail(self) -> None:
        """An absent/blank protocol_version in constraints should not be rejected."""
        from src.scientific_sources.live_query_protocol import (
            load_live_query_protocol,
            validate_complete_authoritative_protocol_projection,
        )
    ...
    def test_unknown_query_id_is_rejected(self) -> None:
        ...
        bad_constraints["queries"][0]["query_id"] = "NOT-A-REAL-QUERY-ID"
...
class TestWorkflowStructure:
    ...
    def test_h2_map_step_has_id_and_tolerates_failure(self) -> None:
        """The H2 map step must expose an id and not hard-fail the job."""
        import yaml  # type: ignore[import-untyped]
    ...
    def test_h2_map_step_sets_has_validated_supply_env(self) -> None:
        """HAS_VALIDATED_SUPPLY must be derived from validated_entries_hydronization_eqf_6_7."""
        wf_path = REPO_ROOT / ".github" / "workflows" / "full-live-analysis.yml"
        content = wf_path.read_text(encoding="utf-8")
        assert "HAS_VALIDATED_SUPPLY=${HAS_VALIDATED}" in content
        assert "validated_entries_hydronization_eqf_6_7" in content

    def test_rebuild_layer5_step_is_conditional_and_passes_supply_map(self) -> None:
        """Layer 5 rebuild must be gated on HAS_VALIDATED_SUPPLY and pass the map path."""
        import yaml  # type: ignore[import-untyped]
    ...
    def test_rebuild_layer5_step_follows_h2_map_step(self) -> None:
        """The conditional rebuild must run after the H2 map is (attempted to be) built."""
        import yaml  # type: ignore[import-untyped]
