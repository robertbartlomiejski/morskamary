...
class TestFingerprintQueryIdHash:
    ...
    def test_normalize_query_constraints_hash_is_order_independent(self) -> None:
        """Query ID hash must depend on the set of IDs, not the order they appear in."""
        mod = _load_script("build_run_stability_report")
    ...
    def test_normalize_query_constraints_no_query_ids_yields_empty_hash(self) -> None:
        """No query_id values present should yield an empty hash, not a spurious digest."""
        mod = _load_script("build_run_stability_report")
        result = mod._normalize_query_constraints({"queries": []})
        assert result["query_id_hash"] == ""

    def test_empty_query_id_hash_falls_back_to_unknown_in_fingerprint(self) -> None:
        """An empty query_id_hash must normalize to 'unknown' in the fingerprint payload."""
        mod = _load_script("build_run_stability_report")
...
class TestProtocolProjectionValidation:
    ...
    def test_missing_queries_list_is_rejected(self) -> None:
        from src.scientific_sources.live_query_protocol import (
            LiveQueryProtocolError,
            load_live_query_protocol,
            validate_complete_authoritative_protocol_projection,
    ...
    def test_unknown_query_id_is_rejected(self) -> None:
        ...
        bad_constraints["queries"][0]["query_id"] = "Q_DOES_NOT_EXIST"
    ...
    def test_sector_slug_mismatch_is_rejected(self) -> None:
        ...
        bad_constraints["queries"][0]["sector_slug"] = "not_a_real_sector"
    ...
    def test_query_text_mismatch_is_rejected(self) -> None:
        ...
        bad_constraints["queries"][0]["query_text"] = "a completely different query"
        with pytest.raises(LiveQueryProtocolError, match="query_text.*mismatch"):
            validate_complete_authoritative_protocol_projection(protocol, bad_constraints)

    def test_blank_protocol_version_in_constraints_is_tolerated(self) -> None:
        """An absent/blank protocol_version in constraints should not itself fail,
        since older artifacts may not have recorded it; the queries are still
        checked in full."""
        from src.scientific_sources.live_query_protocol import (
            load_live_query_protocol,
            validate_complete_authoritative_protocol_projection,
...
class TestWorkflowStructure:
    ...
    def test_rebuild_layer5_step_follows_h2_supply_map_step(self) -> None:
        import yaml  # type: ignore[import-untyped]
