...
class TestWorkflowStructure:
    ...
    def test_h2_map_step_has_id_for_downstream_gating(self) -> None:
        """The H2 map step must expose a step id so later steps can react to it."""
        import yaml  # type: ignore[import-untyped]
    ...
    def test_rebuild_layer5_step_is_gated_on_has_validated_supply(self) -> None:
        """Rebuild Layer 5 must only run when a validated supply map exists."""
        import yaml  # type: ignore[import-untyped]
    ...
    def test_h2_map_step_sets_has_validated_supply_on_every_branch(self) -> None:
        """HAS_VALIDATED_SUPPLY must be exported on the missing-registry,
        missing-output, and computed-map branches so the gate never reads an
        unset environment variable."""
        wf_path = REPO_ROOT / ".github" / "workflows" / "full-live-analysis.yml"
        wf_text = wf_path.read_text(encoding="utf-8")
        assert wf_text.count("HAS_VALIDATED_SUPPLY=") >= 3

    def test_rebuild_layer5_step_follows_h2_map_step(self) -> None:
        """The gated rebuild step must come after the step that computes the
        supply map, otherwise HAS_VALIDATED_SUPPLY would never be set yet."""
        import yaml  # type: ignore[import-untyped]
