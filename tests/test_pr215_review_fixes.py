...
class TestWorkflowStructure:
    ...
    def test_h2_map_step_has_id_and_continues_on_error(self) -> None:
        steps = self._load_workflow_steps()
        h2_step = next(
    ...
    def test_h2_map_step_always_sets_has_validated_supply_env(self) -> None:
        steps = self._load_workflow_steps()
        h2_step = next(
            s for s in steps if s.get("name") == "Build preliminary H2 credential supply map"
        )
        run_script = h2_step.get("run", "")
        assert "HAS_VALIDATED_SUPPLY=" in run_script
        assert "validated_entries_hydronization_eqf_6_7" in run_script

    def test_rebuild_layer5_step_is_conditioned_on_validated_supply(self) -> None:
        steps = self._load_workflow_steps()
        rebuild_step = next(
        ...
        assert "--validated-supply-map" in rebuild_step.get("run", "")
