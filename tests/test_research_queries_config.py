...
def test_research_queries_cover_all_canonical_sectors() -> None:
    ...
    assert isinstance(query_groups, dict) and query_groups, "query_groups must be a mapping"

    labels = []
    for group in query_groups.values():
        assert isinstance(group, dict), "Each query group must be a mapping"
        label = group.get("label")
        assert isinstance(label, str) and label.strip(), "Each query group must define label"
