...
def test_static_recovery_run_is_excluded_from_report(tmp_path: Path) -> None:
    ...
    # Only the non-static-recovery run should have been loaded.
    assert report["runs_analyzed"] == 1
    assert report["saturation_assessment"]["status"] == "not_assessable"


def test_all_static_recovery_runs_yields_zero_runs_analyzed(tmp_path: Path) -> None:
    module = _load_module()
    archive_root = _seed_archive(
        tmp_path,
        [
...
def test_differing_query_id_universes_make_fingerprint_mismatch(tmp_path: Path) -> None:
    """Two runs querying different sets of query_ids must not be treated as
    comparable, even if every other constraint dimension matches."""
    module = _load_module()
    archive_root = _seed_archive(
        tmp_path,
        [
...
def test_same_query_id_universe_keeps_fingerprint_match(tmp_path: Path) -> None:
    """Sanity check: identical query_id sets across runs must still be
    considered comparable (guards against a hash that is too strict)."""
    module = _load_module()
    archive_root = _seed_archive(
        tmp_path,
        [
